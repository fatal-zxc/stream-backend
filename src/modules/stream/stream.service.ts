...
export class StreamService {
	...
	async changeThumbnail(user: User, file: Upload) {
		...
		if (file.filename && file.filename.endWith('.gif')) {
			...
			const processedBuffer = await sharp(buffer).resize(1920, 1080).webp().toBuffer()
	...
	async generateToken(input: GenerateStreamTokenInput) {
		...
		if (user) {
			...
			self = { id: userId, username: `Зритель ${Math.floor(Math.random() * 100_000)}` }
		}
	...
	private async findStreamByUserId(id: string) {
